...
FRAME_ROOT = ROOT / "GenVideo" / "genvidbench_common1000"
...
def main():
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Preparing DeMamba frames"):
        ...
        content_path = f"genvidbench_common1000/{subdir}/{vid_name}"
    ...
    train_df.to_csv(DATASET_DIR / "genvidbench_train.csv", index=False)
    val_df.to_csv(DATASET_DIR / "genvidbench_val.csv", index=False)

    if failed:
        pd.DataFrame(failed).to_csv(DATASET_DIR / "genvidbench_failed.csv", index=False)
