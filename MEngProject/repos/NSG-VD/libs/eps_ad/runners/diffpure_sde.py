...
from libs.eps_ad.guided_diffusion.script_util import create_model_and_diffusion, model_and_diffusion_defaults
from libs.eps_ad.score_sde.losses import get_optimizer
from libs.eps_ad.score_sde.models import utils as mutils
from libs.eps_ad.score_sde.models.ema import ExponentialMovingAverage
...
class RevVPSDE(torch.nn.Module):
	def __init__(self, model, score_type='guided_diffusion', beta_min=0.1, beta_max=20, N=1000, img_shape=(3, 256, 256), model_kwargs=None):
...
class RevGuidedDiffusion(torch.nn.Module):
	def __init__(self, args, config, device=None, process_shape=(3, 256, 256)):
		super().__init__()
		self.args = args
		self.config = config
		if device is None:
		...
		self.model = model
		self.rev_vpsde = RevVPSDE(model=model, score_type=args.score_type, img_shape=img_shape, model_kwargs=None).to(self.device)
